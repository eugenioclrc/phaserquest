...
spaceMap.prototype.getAll = function (fnCall) {
  let l = [];
  for (let i = 0; i < Object.keys(this).length; i++) { // NB: If use forEach instead, "this" won't refer to the object!
    const x = Object.keys(this)[i];
    if (this.hasOwnProperty(x)) {
      for (let j = 0; j < Object.keys(this[x]).length; j++) {
        const y = Object.keys(this[x])[j];
        if (this[x].hasOwnProperty(y)) {
          if (fnCall) {
            for (let k = 0; k < this[x][y].length; k++) {
