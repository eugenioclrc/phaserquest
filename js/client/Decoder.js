...
Decoder.decodeObject = function (pkg, offset, schema) {
  ...
  const propertiesMask = dv[`getUint${schema.propertiesBytes * 8}`](offset); // series of bits indicating the presence or absence of each field of the schema
  ...
  if (schema.arrays) {
    ...
    Object.keys(schema.arrays).forEach((arrayOfObjects) => {
      ...
      if (Decoder.isMaskTrue(propertiesMask, nbProperties, idx)) {
        const length = dv.getUint8(offset); // Number of objects in the array (length of the array)
        offset++;
        if (length) {
    ...
    Object.keys(schema.maps).forEach((map) => {
      if (Decoder.isMaskTrue(propertiesMask, nbProperties, idx)) {
        ...
        if (length) {
          object[map] = {};
          for (let i = 0; i < length; i++) {
            const id = dv[`getUint${CoDec.bytesPerID * 8}`](offset); // ID of the entry (= key)
  ...
  if (schema.booleans) {
    ...
    const bools = dv[`getUint${CoDec.booleanBytes * 8}`](offset); // just like propertiesMask, bools is a mask indicating the presence/absence of each boolean
