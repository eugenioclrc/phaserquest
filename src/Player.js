...
export default class Player extends Human {
  constructor(x, y, key) {
    ...
    super(x, y, key); // Send context as first argument!!
    ...
    this.events.onKilled.add((player) => {
      Game.displayedPlayers.delete(player.id);
    }, this);
  }
  // sets the isPlayer flag to true or false to indicate if a sprite is the main
  ...
  updateLife() { // Update the life bar to reflect the amout of health of the player
    if (this.life < 0) this.life = 0;
    const width = Game.computeLifeBarWidth();
    const tweenWidth = game.add.tween(Game.health.getChildAt(0)); // tween for the "body" of the bar
    const tweenEnd = game.add.tween(Game.health.getChildAt(1)); // tween for the curved tip
    tweenWidth.to({ width }, 200, null, false, 200);
  ...
  teleport() {
    const cell = Game.computeTileCoords(this.x, this.y);
    const door = Game.doors.getFirst(cell.x, cell.y);
    if (door) {
